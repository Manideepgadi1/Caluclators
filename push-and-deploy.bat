...
ssh root@82.25.105.18 "cd /var/www/vsfintech/Investment-Calculator && git pull origin main && pkill -f 'uvicorn app.main:app' && pkill -f 'next start' && cd backend && nohup venv/bin/uvicorn app.main:app --host 0.0.0.0 --port 5003 > /dev/null 2>&1 & cd ../frontend && nohup npm start > /dev/null 2>&1 & sleep 3 && ps aux | grep -E 'uvicorn|next start' | grep -v grep"
